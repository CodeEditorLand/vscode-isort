...
class SortImportsCodeActionProvider implements CodeActionProvider<CodeAction> {
	...
	async provideCodeActions(
		...
		const action1 = new CodeActionWithData(
...
function runDiagnosticsOnStartup(
	serverId: string,
	provider: SortImportsDiagnosticProvider,
) {
	return Promise.all(
...
export function registerSortImportFeatures(
	serverId: string,
): Disposable & { startup: () => Promise<void> } {
