...
export async function getWorkspaceSettings(
	...
	const workspaceSetting = {
		...
		severity: config.get<Record<string, string>>(
			"severity",
			DEFAULT_SEVERITY,
		),
...
export async function getGlobalSettings(
	...
	const setting = {
		...
		severity: getGlobalValue<Record<string, string>>(
			config,
			"severity",
			DEFAULT_SEVERITY,
		),
		interpreter: interpreter ?? [],
		importStrategy: getGlobalValue<string>(
			config,
			"importStrategy",
			"fromEnvironment",
		),
		showNotifications: getGlobalValue<string>(
			config,
			"showNotifications",
			"off",
		),
